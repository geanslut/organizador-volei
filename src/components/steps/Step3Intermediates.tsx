...
export function Step3Intermediates() {
  const { state, dispatch } = useWizard();

  // Filtrar jogadores que não são profissionais
  const availablePlayers = state.players.filter(
    p => !state.selectedProfessionals.includes(p.id)
  );

  const handleToggle = (playerId: string) => {
    dispatch({ type: 'TOGGLE_INTERMEDIATE', payload: playerId });
  };

  const canProceed = state.selectedIntermediates.length === state.numTeams;
  ...
  return (
    <div className="min-h-screen bg-[#212121] relative pb-24 px-6 pt-16 max-w-md mx-auto">
      ...
      <p className="text-[#cecece] text-center font-['DM_Sans',sans-serif] font-light text-[12.893px] leading-[1.19] mb-8" style={{ fontVariationSettings: "'opsz' 14" }}>
        Selecione {state.numTeams} jogador{state.numTeams !== 1 ? 'es' : ''} de suporte
      </p>
      ...
      <div className="bg-[#444] rounded-[33.6px] p-6 min-h-[599px]">
        <p className="text-white text-center font-['Pathway_Extreme',sans-serif] text-[13.768px] leading-[1.08] mb-6" style={{ fontVariationSettings: "'wdth' 100" }}>
          {state.selectedIntermediates.length}/{state.numTeams} selecionados
        </p>

        <div className="grid grid-cols-2 gap-3">
          {availablePlayers.map((player) => {
            const isSelected = state.selectedIntermediates.includes(player.id);
            const isDisabled = !isSelected && state.selectedIntermediates.length >= state.numTeams;
